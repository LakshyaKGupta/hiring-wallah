...
class DatabaseManager:
    ...
    def _decode_row(self, row: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        for key in JSON_COLUMNS:
            if key in data and isinstance(data[key], str):
                try:
                    data[key] = json.loads(data[key] or ("[]" if key in {"strengths", "weaknesses", "evidence", "interview_questions"} else "{}"))
                except json.JSONDecodeError:
                    data[key] = [] if key in {"strengths", "weaknesses", "evidence", "interview_questions"} else {}
        return data
    ...
    async def create_evaluation(self, candidate_id: str, job_id: str, score: int, breakdown: Dict[str, Any], strengths: List[str], weaknesses: List[str], evidence: List[str], devils_advocate: Optional[Dict[str, Any]] = None, resume_id: Optional[str] = None, status: str = "completed", error_message: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def get_job_reports(self, job_id: str) -> List[Dict[str, Any]]:
        return await self._fetchall(
            """
            SELECT reports.*, candidates.name as candidate_name, evaluations.score as score, decisions.ranking as ranking, decisions.verdict as verdict
            FROM reports
            ...
            WHERE reports.job_id=%s ORDER BY COALESCE(decisions.ranking, 999), reports.created_at DESC
            """,
            (job_id,),
        )
    ...
    async def get_job_results(self, job_id: str) -> List[Dict[str, Any]]:
        ...
        for r in rows:
            evaluation = {
                ...
                "evidence": r.get("evidence") or [], "devils_advocate": r.get("devils_advocate") or {},
            ...
            decision = {
                ...
                "verdict": r.get("verdict"), "confidence": r.get("confidence") or 0,
